...
function check_respRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of check_respComponents)
...
function check_trialsLoopBegin(check_trialsLoopScheduler, snapshot) {
  return async function() {
    ...
    // Schedule all the trials in the trialList:
    for (const thisCheck_trial of check_trials) {
...
function def_ac_checkRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of def_ac_checkComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function check_questionRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of check_questionComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function re_def_ac_checkRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of re_def_ac_checkComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
