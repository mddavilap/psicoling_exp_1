...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='D:\\psico_linguistica\\exp1_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    num_activate = int(num_trials * activation_rate)
    ...
    slider_2 = visual.Slider(win=win, name='slider_2',
        ...
        labels=["nada seguro", "muy seguro"], ticks=(1, 2, 3, 4, 5), granularity=1.0,
    ...
    slider_3 = visual.Slider(win=win, name='slider_3',
        startValue=None, size=(1.0, 0.1), pos=(0, 0), units=win.units,
        labels=["sí", "no"], ticks=(1, 2), granularity=0.0,
        style='rating', styleTweaks=(), opacity=None,
        labelColor='LightGray', markerColor='Red', lineColor='White', colorSpace='rgb',
        font='Open Sans', labelHeight=0.05,
        flip=False, ori=0.0, depth=-1, readOnly=False)
    ...
    thisExp.addData('check_resp.started', globalClock.getTime(format='float'))
    # keep track of which components have finished
    check_respComponents = []
    for thisComponent in check_respComponents:
    ...
    thisExp.addData('check_resp.stopped', globalClock.getTime(format='float'))
    thisExp.nextEntry()
    ...
    _key_resp_allKeys = []
    # keep track of which components have finished
    instructionsComponents = [inst, key_resp]
    ...
    while continueRoutine:
        ...
        # if inst is starting this frame...
    ...
    for thisTrial in trials:
        ...
        q.setSound(question, hamming=True)
        q.setVolume(800.0, log=False)
        q.seek(0)
        # keep track of which components have finished
        questionComponents = [q, text]
        ...
        while continueRoutine:
            ...
            if q.status == NOT_STARTED and t >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                ...
                # add timestamp to datafile
                thisExp.addData('q.started', t)
                # update status
                q.status = STARTED
                q.play()  # start the sound (it finishes automatically)
            # update q status according to whether it's playing
            if q.isPlaying:
                q.status = STARTED
            elif q.isFinished:
                q.status = FINISHED
            ...
            if text.status == STARTED:
                ...
                if tThisFlipGlobal > text.tStartRefresh + q.getDuration()-frameTolerance:
        ...
        q.pause()  # ensure sound has stopped at end of Routine
        ...
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('time.started', globalClock.getTime(format='float'))
        te.setSound('questions/blanco.wav', secs=time, hamming=True)
        te.setVolume(8000.0, log=False)
        te.seek(0)
        # keep track of which components have finished
        timeComponents = [te]
        for thisComponent in timeComponents:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        while continueRoutine:
            ...
            # if te is starting this frame...
            if te.status == NOT_STARTED and t >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                ...
                # add timestamp to datafile
                thisExp.addData('te.started', t)
                # update status
                te.status = STARTED
                te.play()  # start the sound (it finishes automatically)
            
            # if te is stopping this frame...
            if te.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > te.tStartRefresh + time-frameTolerance:
                    # keep track of stop time/frame for later
                    te.tStop = t  # not accounting for scr refresh
                    te.tStopRefresh = tThisFlipGlobal  # on global time
                    te.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.addData('te.stopped', t)
                    # update status
                    te.status = FINISHED
                    te.stop()
            # update te status according to whether it's playing
            if te.isPlaying:
                te.status = STARTED
            elif te.isFinished:
                te.status = FINISHED
        ...
        for thisComponent in timeComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('time.stopped', globalClock.getTime(format='float'))
        te.pause()  # ensure sound has stopped at end of Routine
        ...
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('answer.started', globalClock.getTime(format='float'))
        aw.setSound(answer, hamming=True)
        aw.setVolume(8000.0, log=False)
        aw.seek(0)
        # keep track of which components have finished
        answerComponents = [aw, text_3]
        ...
        while continueRoutine:
            ...
            # if aw is starting this frame...
            if aw.status == NOT_STARTED and t >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                aw.frameNStart = frameN  # exact frame index
                aw.tStart = t  # local t and not account for scr refresh
                aw.tStartRefresh = tThisFlipGlobal  # on global time
                # add timestamp to datafile
                thisExp.addData('aw.started', t)
                # update status
                aw.status = STARTED
                aw.play()  # start the sound (it finishes automatically)
            # update aw status according to whether it's playing
            if aw.isPlaying:
                aw.status = STARTED
            elif aw.isFinished:
                aw.status = FINISHED
            ...
            if text_3.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > text_3.tStartRefresh + aw.getDuration()-frameTolerance:
                    # keep track of stop time/frame for later
        ...
        for thisComponent in answerComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('answer.stopped', globalClock.getTime(format='float'))
        aw.pause()  # ensure sound has stopped at end of Routine
        ...
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('slider.started', globalClock.getTime(format='float'))
        slider_2.reset()
        # keep track of which components have finished
        sliderComponents = [text_2, slider_2]
        ...
        for thisComponent in sliderComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('slider.stopped', globalClock.getTime(format='float'))
        ...
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('def_ac_check.started', globalClock.getTime(format='float'))
        # Run 'Begin Routine' code from code_4
        activate_extra_routine = activation_list[trial_index]
        # keep track of which components have finished
        def_ac_checkComponents = []
        for thisComponent in def_ac_checkComponents:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        for thisComponent in def_ac_checkComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('def_ac_check.stopped', globalClock.getTime(format='float'))
        # the Routine "def_ac_check" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        
        # set up handler to look after randomisation of conditions etc
        check_trials = data.TrialHandler(nReps=activate_extra_routine, method='random', 
            extraInfo=expInfo, originPath=-1,
            trialList=[None],
            seed=None, name='check_trials')
        thisExp.addLoop(check_trials)  # add the loop to the experiment
        thisCheck_trial = check_trials.trialList[0]  # so we can initialise stimuli with some values
        # abbreviate parameter names if possible (e.g. rgb = thisCheck_trial.rgb)
        if thisCheck_trial != None:
            for paramName in thisCheck_trial:
                globals()[paramName] = thisCheck_trial[paramName]
        
        for thisCheck_trial in check_trials:
            currentLoop = check_trials
            thisExp.timestampOnFlip(win, 'thisRow.t', format=globalClock.format)
            # pause experiment here if requested
            if thisExp.status == PAUSED:
                pauseExperiment(
            ...
            continueRoutine = True
            # update component parameters for each repeat
            thisExp.addData('check_question.started', globalClock.getTime(format='float'))
            text_4.setText(check)
            slider_3.reset()
            # keep track of which components have finished
            check_questionComponents = [text_4, slider_3]
            for thisComponent in check_questionComponents:
                thisComponent.tStart = None
                thisComponent.tStop = None
                thisComponent.tStartRefresh = None
                thisComponent.tStopRefresh = None
            ...
            for thisComponent in check_questionComponents:
                if hasattr(thisComponent, "setAutoDraw"):
                    thisComponent.setAutoDraw(False)
            thisExp.addData('check_question.stopped', globalClock.getTime(format='float'))
            check_trials.addData('slider_3.response', slider_3.getRating())
            check_trials.addData('slider_3.rt', slider_3.getRT())
        ...
        # completed activate_extra_routine repeats of 'check_trials'
        
        
        # --- Prepare to start Routine "re_def_ac_check" ---
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('re_def_ac_check.started', globalClock.getTime(format='float'))
        # Run 'Begin Routine' code from code_5
        trial_index += 1
        # keep track of which components have finished
        re_def_ac_checkComponents = []
        for thisComponent in re_def_ac_checkComponents:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        for thisComponent in re_def_ac_checkComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('re_def_ac_check.stopped', globalClock.getTime(format='float'))
